...
public class DependencyGraph {
    ...
    private List<EquationalBlock> blocks;

    /* nu X1 -> fixedPointVarMap.get("X1") returns the node associated to nu X1 */
    private Map<String, FormulaNode> fixedPointVarMap;
    ...
    private void setVarNumbers(FormulaNode root) {
        AtomicInteger numVars = new AtomicInteger(0);
        setVarNumbers(root, numVars);
        this.numVars = numVars.get();
    }
    ...
    private void setVarNumbers(FormulaNode node, AtomicInteger numVars) {
        ...
        if (node instanceof VariableNode) {
            /* VariableNode has same variableNumber as its the fixed point it references */
